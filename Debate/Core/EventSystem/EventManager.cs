...
namespace Debate.Core.EventSystem
{
    public class EventManager
    {
        ...
        //OnceLookups are stored listeners, simiilar to delegate lookup, to indicate a listener that can only be actioned once.
        private readonly Dictionary<Delegate, Delegate> _onceLookups = new Dictionary<Delegate, Delegate>();
        #endregion
        ...
        public void RemoveListener<T>(EventDelegate<T> del) where T : GameEvent
        {
            ...
            {
                Console.WriteLine("Deletegate of type " + typeof(T) + "was not found when removing a listener.");
                return;
            }
        ...
        public void TriggerEvent(GameEvent e)
        {
            ...
            if (_delegates.TryGetValue(eventType, out del))
            {
                ...
                Console.WriteLine("Event: " + eventType + " has no listeners");
        }
        ...
        public bool QueueEvent(GameEvent evt)
        {
            ...
            if (!_delegates.ContainsKey(evt.GetType()))
            {
                ...
                Console.WriteLine("EventManager: GetEventToQueue failed due to no listeners for event: " + evt.GetType());
                return false;
            }
        ...
        private void RemoveOnceLookUpListener(EventDelegate internalDelegate)
        {
            Delegate tempDel;
        ...
        private void RemoveActionedOnceLookups(Type typeKey)
        {
            ...
            foreach (var @delegate in foundDelgate.GetInvocationList())
            {
        ...
        private void RemoveDelegateListener<T>(EventDelegate internalDelegate) where T : GameEvent
        {
            EventDelegate tempDel;
            //If there is no delegates with Type t, return null.
            if (!_delegates.TryGetValue(typeof(T), out tempDel)) return;
